...
최종 점수는 `calculate_final_score` 함수에서 다음과 같이 계산됩니다:

1.  **초기 가중치 정의**: `factor_weights` 딕셔너리에 각 점수(`value_score`, `quality_score`, `momentum_score`, `supply_score`, `volatility_score`, `ml_pred_proba`, `sentiment_score`, `dl_trend_score(더미)`)에 대한 초기 가중치가 정의되어 있습니다. 현재 설정된 가중치는 다음과 같습니다:
    *   `value_score`: 0.20
    *   `quality_score`: 0.20
    *   `momentum_score`: 0.30
    *   `supply_score`: 0.00
...
2.  **활성 가중치 추출 및 정규화**: `factor_weights` 중에서 가중치가 0보다 크고 입력 데이터프레임에 해당 컬럼이 존재하는 '활성' 요소들만 추출합니다. 이 활성 요소들의 가중치 합계가 1이 되도록 정규화합니다. 즉, 가중치가 0인 요소는 최종 점수 계산에 기여하지 않으며, 나머지 활성 요소들의 가중치가 상대적으로 조정됩니다.

3.  **최종 점수 계산**: 각 활성 요소의 점수(데이터프레임의 해당 컬럼 값)에 정규화된 가중치를 곱한 후, 이 값들을 모두 더하여 `final_score`를 계산합니다. 이때, 누락된 점수는 0으로 처리됩니다 (`fillna(0)`).

4.  **0-100 스케일링**: 계산된 `final_score`는 최소-최대 정규화(Min-Max Normalization)를 통해 0에서 100 사이의 값으로 스케일링됩니다. 만약 모든 점수가 동일하여 최대값과 최소값이 같을 경우, `final_score`는 50으로 설정됩니다.

5.  **최종 순위 및 반올림**: 마지막으로 `final_score`를 기준으로 내림차순으로 `최종순위`가 부여되며, `final_score`는 소수점 둘째 자리까지 반올림됩니다.
