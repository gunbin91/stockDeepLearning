...
-   **가치(Value) 지표**:
    -   `PBR` (주가순자산비율), `PER` (주가수익비율), `BPS` (주당순자산), `이익수익률` (Earnings Yield)
-   **퀄리티(Quality) 지표**:
    -   `PBR` (주가순자산비율) - 품질 점수 계산에 활용
-   **모멘텀(Momentum) 지표**:
    -   `수익률(1M)`, `수익률(3M)`: 1개월, 3개월 주가 수익률
    -   `52주_신고가_비율`: 52주 최고가 대비 현재 주가 비율
    -   `disparity_120`, `disparity_240`: 주가와 120일, 240일 이동평균선 간의 이격도
