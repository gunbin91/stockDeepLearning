...
# StandardScaler 제거 (CatBoost는 스케일링 불필요)
...
def objective(trial, fold_data_cache, features):
    ...
    params = {
        ...
        'loss_function': 'Logloss',
        'eval_metric': 'Logloss' if device == 'GPU' else 'AUC',
        'iterations': 3000,
        ...
        # [수정 1] colsample_bylevel 추가: 피처 쏠림 방지의 핵심
        # 매 분기마다 전체 피처의 50% ~ 90%만 랜덤하게 사용 -> 약한 피처도 학습 기회 획득
        'colsample_bylevel': trial.suggest_float('colsample_bylevel', 0.5, 0.95),
        
        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.2, log=True),
        'depth': trial.suggest_int('depth', 5, 10),
        
        # [수정 2] L2 규제 상한선 조정 (10.0 -> 5.0)
        # 너무 강한 규제는 약한 피처를 0으로 만듦
    ...
    log_info(f"   📋 파라미터: learning_rate={params['learning_rate']:.6f}, depth={params['depth']}, "
             ...
             f"colsample_bylevel={params['colsample_bylevel']:.2f}")
    ...
    for fold_idx, (X_train, y_train, X_val, y_val) in fold_data_cache.items():
        try:
            ...
            # --- 스케일링 제거됨 (CatBoost는 스케일링 불필요) ---
            
            # CatBoost Pool 생성 (스케일링 되지 않은 원본 데이터 사용)
            # [수정 3] feature_names 명시적 전달
            train_pool = Pool(data=X_train_resampled, label=y_train_resampled, feature_names=features)
            val_pool = Pool(data=X_val, label=y_val, feature_names=features)
...
def train_final_model(fold_ranges, file_paths, features, best_params, best_score=None, n_trials=None, trials_completed=None):
    ...
    # --- 클래스 불균형 확인 및 언더샘플링 (RF/LGBM과 동일하게 1:1) ---
    ...
    # CatBoost Pool 생성 (스케일링 되지 않은 원본 데이터 사용)
    # [수정 4] 최종 학습 시에도 feature_names 명시
    train_pool = Pool(data=X_all_scaled, label=y_all, feature_names=available_features)
    ...
    if val_data_df is not None and not val_data_df.empty:
        ...
        # 스케일링 되지 않은 원본 데이터 사용
        X_val_scaled = X_val
        # [수정 4] 검증셋에도 feature_names 명시
        val_pool = Pool(data=X_val_scaled, label=y_val, feature_names=available_features)
    ...
    try:
        ...
        # 평가용 데이터 준비
        if val_pool is not None and X_val is not None and y_val is not None:
            # X_val은 이미 스케일링 되지 않은 상태임
            X_eval = X_val
            y_eval = y_val
            ...
            # X_all_scaled는 스케일링 되지 않은 X_all임
            if isinstance(X_all_scaled, pd.DataFrame):
...
def main():
    ...
    # 사용자 입력 (Trial 횟수)
    try:
        trials_input = input("\n   [CatBoost] Optuna Trial 횟수를 입력하세요 (기본값: 50, Enter): ").strip()
