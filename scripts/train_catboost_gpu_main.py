...
def check_gpu_availability():
    ...
    try:
        ...
        test_params = {
            ...
            'eval_metric': 'Logloss',  # GPU에서는 Logloss 사용 (AUC는 GPU에서 직접 지원 안 됨)
            'iterations': 1,
            'verbose': False,
            'random_seed': 42,
        ...
        # GPU 학습 시도
        test_model = CatBoostClassifier(**test_params)
        test_model.fit(dummy_X, dummy_y)
        
        # 성공하면 GPU 사용 가능
        check_gpu_availability._gpu_available = True
        check_gpu_availability._device = 'GPU'
        log_info("   ✅ GPU 사용 가능: CatBoost가 GPU로 학습합니다.")
        return True, 'GPU'
...
def objective(trial, fold_data_cache, features):
    """
    Optuna를 위한 objective 함수 (CatBoost).
    ...
    # 하이퍼파라미터 제안
    # GPU 모드에서는 AUC가 직접 지원되지 않아 CPU로 폴백되므로,
    # GPU 모드일 때는 Logloss를 사용하고, 최종 평가는 별도로 AUC 계산
    params = {
        'task_type': device,
        'devices': '0' if device == 'GPU' else None,
        'loss_function': 'Logloss',
        'eval_metric': 'Logloss' if device == 'GPU' else 'AUC',  # GPU에서는 Logloss 사용
        'iterations': 3000,
    ...
    # [개선] patience를 learning_rate에 비례시킨다 (기존 고정 50은 너무 짧음)
    params['early_stopping_rounds'] = int(min(600, max(200, 5.0 / params['learning_rate'])))
    log_info(f"   ⏱️ early_stopping_rounds = {params['early_stopping_rounds']} (lr={params['learning_rate']:.5f})")
    
    log_info(f"   📋 파라미터: learning_rate={params['learning_rate']:.6f}, depth={params['depth']}, "
             f"l2_leaf_reg={params['l2_leaf_reg']:.6f}, border_count={params['border_count']}")
    ...
    for fold_idx, cache_item in fold_data_cache.items():
        try:
            ...
            # CatBoost Pool 생성
            train_pool = Pool(data=X_train_scaled, label=y_train_resampled, weight=w_train_resampled)
            ...
            try:
                ...
                # GPU 실패 시 CPU로 전환
                if 'GPU' in str(gpu_error) or 'gpu' in str(gpu_error).lower() or 'device' in str(gpu_error).lower():
                    # CPU 파라미터로 변경
            ...
            # [진단] 폴드별 조기종료 시점 기록
            fold_best_iters.append(int(getattr(model, 'best_iteration_', 0) or 0))
            log_info(
                f"   🔎 Fold #{fold_idx+1} best_iteration={getattr(model, 'best_iteration_', None)}, "
                f"dailyAUC={daily_auc:.4f}, prec@15={prec_k:.4f}, mixed={auc_score:.4f}, "
                f"lr={params['learning_rate']:.5f}"
            )
            
            # 메모리 정리 (스케일링된 데이터 등)
            del X_train_scaled, X_val_scaled, train_pool, val_pool, model
...
def train_final_model(fold_ranges, file_paths, features, best_params, best_score=None, n_trials=None, trials_completed=None, final_num_rounds=None, train_start_date=None, train_end_date=None):
    ...
    require_sample_meta(type_all, w_all, need_weight=True, context="CatBoost 최종학습")
    before_n = len(y_all)
    ...
    # [개선] Inf만 NaN으로. 결측치는 CatBoost 네이티브 처리에 맡긴다.
    ...
    final_params.update({
        ...
        'loss_function': 'Logloss',
        'eval_metric': 'Logloss' if device == 'GPU' else 'AUC',  # GPU에서는 Logloss 사용
        'iterations': int(final_num_rounds) if final_num_rounds else 5000,
        'early_stopping_rounds': 50,
        'verbose': True,
        'random_seed': 42,
        'allow_writing_files': False,
    })
    # 고정 라운드 모드에서는 early_stopping_rounds 제거
    if final_num_rounds is not None:
        final_params.pop('early_stopping_rounds', None)
    
    # CatBoost Pool 생성
    train_pool = Pool(data=X_all_scaled, label=y_all, weight=w_all)
    
    # 검증 데이터셋 준비 (고정 라운드 모드에서는 ES용 홀드아웃 생략)
    val_pool = None
    X_val_scaled = None
    y_val = None
    ...
    try:
        ...
        if 'GPU' in str(gpu_error) or 'gpu' in str(gpu_error).lower() or 'device' in str(gpu_error).lower():
            ...
            final_params_cpu['task_type'] = 'CPU'
            final_params_cpu['devices'] = None
            # GPU 사용 불가능 플래그 설정
            check_gpu_availability._gpu_available = False
            check_gpu_availability._device = 'CPU'
            # CPU로 재시도
            final_model = CatBoostClassifier(**final_params_cpu)
            if final_num_rounds is not None:
    ...
    elif hasattr(final_model, 'best_iteration_') and final_model.best_iteration_ is not None:
        log_info(f"   ✅ 최종 모델 학습 완료 (best_iteration: {final_model.best_iteration_})")
        best_iteration = final_model.best_iteration_
    else:
        log_info(f"   ✅ 최종 모델 학습 완료 (총 {final_params['iterations']} iterations)")
        best_iteration = final_params['iterations']
    ...
    # 1. 기본 중요도 (CatBoost 내장)
    try:
        gain_importance = final_model.get_feature_importance()
    ...
    try:
        ...
        # 평가용 데이터 준비
        if val_pool is not None and X_val_scaled is not None and y_val is not None:
            X_eval = X_val_scaled
            y_eval = y_val
            log_info(f"   [PERM] 검증 데이터 사용: {len(X_eval):,}행")
        else:
            # 학습 데이터 일부 샘플링
        ...
        log_info(f"   [OK] 순열 중요도 평균: {results.importances_mean.mean():.6f}, 최대: {results.importances_mean.max():.6f}")
        
    except Exception as e:
        log_warning(f"   ⚠️ 순열 중요도 계산 실패: {e}")
        import traceback
        log_warning(f"   ⚠️ 순열 중요도 계산 실패 상세: {traceback.format_exc()}")
...
def main():
    """메인 실행 함수"""
    parser = argparse.ArgumentParser(description="CatBoost GPU 가속 모델 훈련 스크립트")
    parser.add_argument('--n_iter', type=int, default=20, help='Optuna 탐색 횟수')
    args = parser.parse_args()
    
    # 사용자 입력 (Trial 횟수)
    try:
        trials_input = input("\n   [CatBoost] Optuna Trial 횟수를 입력하세요 (기본값: 20, Enter): ").strip()
    ...
    # RF/LGBM과 동일한 피처 목록 (실제 계산되는 피처만 포함)
    features = [
        'log_mktcap_cs',
        '52주_신고가_비율',
        'ADX_14_cs',
    ...
    fold_data_cache = {}  # {fold_idx: (X_train, y_train, w_train, type_train, X_val, y_val)}
    ...
    for fold_info in fold_ranges:
        ...
        try:
            ...
            # (CatBoost는 NaN을 네이티브 처리 — 추론 경로와 동일한 전처리 유지)
