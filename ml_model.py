...
RandomForest 모델을 사용하여 아래 타겟(이진 분류)을 만족할 확률을 계산합니다:
...
def predict_with_ml_model(df):
    ...
    훈련된 RandomForest 모델을 사용하여 각 종목의 타겟 달성 확률을 예측합니다.
    (타겟: 향후 10거래일 동안 -5% 이상 하락 없이, +8% 이상 상승을 한 번이라도 달성 여부)
...
def predict_with_lgb_model(df):
    """
    LightGBM 모델을 사용한 상승 확률 예측 함수
    
    훈련된 LightGBM 모델을 사용하여 각 종목의 타겟 달성 확률을 예측합니다.
    (타겟: 향후 10거래일 동안 -5% 이상 하락 없이, +8% 이상 상승을 한 번이라도 달성 여부)
    데이터 전처리(스케일링, 결측값 처리)를 자동으로 수행합니다.
    ...
    if df.empty:
        log_warning("입력 데이터프레임이 비어있습니다.")
        return pd.DataFrame(columns=['종목코드', 'lgb_pred_proba'])
    ...
    lgb_model_path = path_manager.get_lgb_model_path()
    if not os.path.exists(lgb_model_path):
        log_warning(f"LightGBM 모델 파일('{lgb_model_path}')을 찾을 수 없습니다. LightGBM 예측을 건너뜁니다.")
        result_df['lgb_pred_proba'] = np.nan
        return result_df

    try:
        log_info(f"🤖 LightGBM 모델 예측 중... ({len(df):,}개 종목)")
        model_data = joblib.load(lgb_model_path)
        model = model_data['model']
        features = model_data['features']
        scaler = model_data['scaler']
        imputation_values = model_data['imputation_values']
    except Exception as e:
        error_msg = f"LightGBM 모델 파일('{lgb_model_path}')을 로드하는 중 문제가 발생했습니다: {e}"
    ...
    try:
        ...
        X_pred_scaled = pd.DataFrame(X_pred_scaled, columns=features)
    except Exception as e:
        log_warning("   ⚠️ 스케일링 오류, 원본 데이터 사용")
        X_pred_scaled = X_pred
    ...
    try:
        ...
        error_msg = f"LightGBM 모델 예측 중 오류 발생: {e}"
        log_warning(error_msg)
        result_df['lgb_pred_proba'] = np.nan
        return result_df
    
    result_df['lgb_pred_proba'] = y_pred_proba
    log_info("🎉 LightGBM 예측 완료!")
